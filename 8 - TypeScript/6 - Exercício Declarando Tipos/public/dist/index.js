...
function addCrewMember(member, spaceship) {
    if (spaceship.crew.length >= spaceship.crewLimit) {
        alert(`${member} não pôde embarcar. Limite atingido`);
    }
    else {
        spaceship.crew.push(member);
        alert(`${member} juntou-se à tripulação da ${spaceship.name}`);
    }
}
...
function firstMenuOption() {
    ...
    const pilot = prompt(`Quem será o piloto da ${name}`);
    const crewLimit = Number.parseInt(prompt(`Quantos tripulantes a ${name} suporta?`));
    const confirmation = confirm(prompt(`Confirma o registro da nave ${name}?\nPiloto: ${pilot}\nTamanho da Tripulação: ${crewLimit}`));
    if (confirmation) {
